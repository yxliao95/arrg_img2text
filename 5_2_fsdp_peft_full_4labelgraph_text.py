...
class GlobalVariables:
    ...
    graph_tokens = ["<normal>", "<abnormal>", "<absent>", "<uncertain>", "<suggestive_of>", "located_at"]
...
def get_inputs_for_training(tokenizer, batch_data, pixel_values, image_indices_map):
    ...
    for idx, item in enumerate(batch_data):
        ...
        for graph_repr in item["graph_reprs2"]:
            for key in ["normal", "abnormal", "uncertain", "absent"]:
                if not graph_repr[key]:
                    continue
                assert len(graph_repr[key]) == 1
                graph_str_dict[key].append(graph_repr[key][0].lower())
...
def get_inputs_for_inference(tokenizer, batch_data, pixel_values, image_indices_map, assistant_responses, conversations_history):
    ...
    for idx, item in enumerate(batch_data):
        ...
        for graph_repr in item["graph_reprs2"]:
            ...
            # {'abnormal': ['hazy opacity <located_at> overlies <located_at> lungs'], 'absent': [], 'normal': [], 'uncertain': []}
            for key in ["normal", "abnormal", "uncertain", "absent"]:
                if not graph_repr[key]:
                    continue
