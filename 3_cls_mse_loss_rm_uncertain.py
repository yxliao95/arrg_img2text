#############################################
# 基于 3_cls_effu_regression_weighted_loss.py
# 改进：
# 在训练过程中，移除uncertain标签和相关数据，只保留present和absent标签；评估是依然保留uncertain标签
#############################################
import argparse
import datetime
import glob
...
def load_preprocessed_dataset(ds_path):
    ...
    ds_final_train = ds_final["train"]
    # remove uncertain samples from training set
    selected_indices = [idx for idx, label in enumerate(ds_final_train["effusion_label"]) if label != 0.5]
    filtered_ds_final_train = ds_final_train.select(selected_indices)
    ds_final["train"] = filtered_ds_final_train
