...
from ..constants import EXTRA_CACHE_DIR
from ..RadEntityMatchExact import RadEntityMatchExact
from ..zoo.utils import download_model
...
class RadEntityNLI(nn.Module):
    def __init__(self, **kwargs):
        ...
        if not os.path.exists(checkpoint):
            download_model(repo_id="StanfordAIMI/RRG_scorers", cache_dir=EXTRA_CACHE_DIR, filename="model_medrad_19k.gz")

        # NLI scorer
        model = SimpleNLI.load_model(checkpoint)
        self.nli = SimpleNLI(model, batch=24, neutral_score=0.3333333333333333, nthreads=2, pin_memory=False, bert_score="distilbert-base-uncased", cache=200000, verbose=False)

        # BertSore
        self.bert_scorer = BERTScorer(model_type="distilbert-base-uncased", num_layers=5, batch_size=64, nthreads=4, all_layers=False, idf=False, device="cuda", lang="en", rescale_with_baseline=True, baseline_path=None)
    ...
    def forward(self, refs, hyps):
        ...
        with torch.no_grad():
            ...
            for doc_h, doc_r in zip(docs_h, docs_r):
                ...
                hyp_report = [" ".join([token["text"] for token in sentence.to_dict()]) for sentence in doc_h.sentences]
                ref_report = [" ".join([token["text"] for token in sentence.to_dict()]) for sentence in doc_r.sentences]

                if len(hyp_report) == 0 or len(ref_report) == 0:
                    continue

                ner_h = [[ner["text"] for ner in sentence.to_dict() if ner["ner"] in self.target_types] for sentence in doc_h.sentences]
                ner_r = [[ner["text"] for ner in sentence.to_dict() if ner["ner"] in self.target_types] for sentence in doc_r.sentences]
...
if __name__ == "__main__":
    ...
    x = RadEntityNLI()(refs=l2, hyps=l1)
